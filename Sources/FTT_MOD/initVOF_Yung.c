...
void initVOF(int itNb)
{
  ...
  for (iCell = 0; iCell < cellNumber; iCell++)
  {
    ...
    {
      vof[iCell] = initcomputeVOF(iCell);
    }
  }
}

Real initcomputeVOF(int iCell)
{
  ...
  {
    printf("box %d %g %g \n", i, xcoor[i], ycoor[i]);
    color[i] = (xcoor[i] - xc) * (xcoor[i] - xc) + (ycoor[i] - yc) * (ycoor[i] - yc) - radius * radius;
  }
  if (color[0] < 0.0)
    if (color[1] < 0.0)
      if (color[2] < 0.0)
