...
void binCollection(void)
{
  int id, iCell, iLv, iOct, iFlag, octCell, chOct, child, prCell;
  FILE *fp;

//  printf("before binCollection: numberOfOcts = %d\n", numberOfOcts);
  for(iOct=1; iOct<numberOfOcts; iOct++)
  {
    octCell = 4*iOct;
    if(octFlag[iOct]==0 && octLv[iOct] == maxLevel)
    {
//      printf("iOct = %d level = %d\n", iOct, octLv[iOct]);
      numberOfOcts--;
      iCell = numberOfOcts*4;
      while(octFlag[numberOfOcts]==0 && octLv[numberOfOcts] == maxLevel)
      {
        numberOfOcts--;
        iCell = numberOfOcts*4;
      }
...
// copy vof
  ...
  if(iOct > numberOfOcts) numberOfOcts++;
...
*/
//  printf("after binCollection: numberOfOcts = %d\n", numberOfOcts);
}
